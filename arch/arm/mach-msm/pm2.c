...
enum {
	...
	MSM_PM_DEBUG_VIBRATE_ON_WAKE = 1U << 7,
};
...
//		writel(0,MSM_GPIOCFG1_BASE+0x220); // disable  keysense IRQ
	...
	pm_saved_acpu_clk_rate = 128 * 1000;
/*
...
enter_failed:
	...
	if(msm_pm_debug_mask & MSM_PM_DEBUG_VIBRATE_ON_WAKE) { // vibrate to test pc
	...
	if (sleep_time < msm_pm_idle_sleep_min_time || !allow_sleep) {
		...
		if (msm_pm_debug_mask & MSM_PM_DEBUG_CLOCK)
			printk(KERN_DEBUG "msm_sleep: clk swfi -> %ld\n",
				saved_rate);
		if (saved_rate && acpuclk_set_rate(saved_rate, SETRATE_PC) < 0)
			printk(KERN_ERR "msm_sleep(): clk_set_rate %ld "
			       "failed\n", saved_rate);
#ifdef CONFIG_MSM_IDLE_STATS
