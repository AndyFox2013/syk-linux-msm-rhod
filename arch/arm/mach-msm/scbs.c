...
static int scbs_probe(struct platform_device *pdev)
{
	//GTG
	return 0;
}

static int scbs_suspend(struct platform_device *pdev, pm_message_t mesg)
{
	if(!use_scbs)
		return 0;
	kill_timer = 1;
...
static int scbs_resume(struct platform_device *pdev, pm_message_t mesg)
{
	if(use_scbs)
	{
		kill_timer = 0;
...
static struct platform_driver scbs_pm_driver = {
	.probe		= scbs_probe,
	.suspend	= scbs_suspend,
	.resume		= scbs_resume,
	.driver		= {
		.name		= "scbs_pm",
		.owner		= THIS_MODULE,
...
static struct platform_device scbs_pm_device = {
	.name = "scbs_pm",
	.id = -1,
};
...
int scbs_init_devices(void)
{
	...
	rc = quickwakeup_register(&quick_ops);
	if(rc < 0)
		goto fail_destroy_device;

	rc = platform_driver_register(&scbs_pm_driver);
 	if(rc < 0)
		goto fail_kill_quickwakeup;

	rc = platform_device_register(&scbs_pm_device);
	if(rc<0)
		goto fail_kill_quickwakeup;

	return rc;

fail_kill_quickwakeup:
	quickwakeup_unregister(&quick_ops);
fail_destroy_device:
