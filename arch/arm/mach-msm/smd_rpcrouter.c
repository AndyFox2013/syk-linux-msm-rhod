...
static int rpcrouter_send_control_msg(union rr_control_msg *msg)
{
	...
	if (!(msg->cmd == RPCROUTER_CTRL_CMD_HELLO || msg->cmd == RPCROUTER_CTRL_CMD_BYE) && !initialized) {
...
//		new_server(0x3000000b,0); // register this for adsp
	...
	case RPCROUTER_CTRL_CMD_NEW_SERVER:
		RR("o NEW_SERVER id=%d:%08x prog=%08x:%x\n",
...
int msm_rpc_write(struct msm_rpc_endpoint *ept, void *buffer, int count)
{
	...
	if (rq->type == 0) {
		...
		IO("CALL to %08x:%d @ %d:%08x (%d bytes) (xid %x proc %x)\n",
		   be32_to_cpu(rq->prog), be32_to_cpu(rq->vers),
		   ept->dst_pid, ept->dst_cid, count, be32_to_cpu(rq->xid), be32_to_cpu(rq->procedure));
	} else {
		/* RPC REPLY */
		/* TODO: locking */
