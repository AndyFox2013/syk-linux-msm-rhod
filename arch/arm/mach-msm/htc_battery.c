...
#ifdef CONFIG_SCBS
extern int scbs_capacity;
extern int use_scbs;
#else
int use_scbs=0;
int scbs_capacity=50;
#endif
...
//	mutex_lock(&htc_batt_info.lock);
	...
	if (htc_batt_info.resources->smem_field_size == 4) {
		...
		buffer->batt_discharge = batt_32->batt_discharge;
...
EXPORT_SYMBOL(htc_get_batt_smem_info);
...
//	}
	...
	if (mutex_is_locked(&htc_batt_info.lock))
		mutex_unlock(&htc_batt_info.lock);
...
dont_need_update:
	mutex_lock(&htc_batt_info.lock);
	if(use_scbs)
		capacity = scbs_capacity;
	else
		capacity = htc_batt_info.rep.batt_vol;
