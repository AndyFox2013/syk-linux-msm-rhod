...
static int gpio_chip_direction_input(struct gpio_chip *chip, unsigned offset)
{
	...
	old_chip = container_of(chip, struct old_gpio_chip, old_chip);
...
int register_gpio_chip(struct old_gpio_chip *new_gpio_chip)
{
	...
	new_gpio_chip->old_chip.direction_input = gpio_chip_direction_input;
	new_gpio_chip->old_chip.get = gpio_chip_get;
	new_gpio_chip->old_chip.direction_output = gpio_chip_direction_output;
	new_gpio_chip->old_chip.set = gpio_chip_set;
	new_gpio_chip->old_chip.to_irq = gpio_chip_to_irq;
	new_gpio_chip->old_chip.base = new_gpio_chip->start;
	new_gpio_chip->old_chip.ngpio =
		new_gpio_chip->end - new_gpio_chip->start + 1;

	return gpiochip_add(&new_gpio_chip->old_chip);
}
