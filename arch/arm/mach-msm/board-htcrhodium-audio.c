...
static int __init rhod_audio_init(void) {
	...
	printk(KERN_INFO "Rhodium audio registering drivers\n");
        gpio_request(RHODIUM_HS_AMP_PWR, NULL);
...
void htcrhodium_set_headset_amp(bool enable) {
    if (enable)
    {
        ...
        gpio_direction_output(RHODIUM_HS_AMP_PWR, 1);
    }
    else
    {
        /* Power down headphone amp */
        gpio_direction_output(RHODIUM_HS_AMP_PWR, 0);
    }
...
static void htcrhodium_microp_audio_leds_update(struct work_struct* work)
{
	int ret;
...
void htcrhodium_microp_audio_dualmic_set(char on)
{
	printk(KERN_DEBUG "%s(%d)\n", __func__, on);
	if (on)
		audio_data.states |= RHOD_MICROP_KLT_DUALMIC_EN_BIT;
	else
		audio_data.states &= (~RHOD_MICROP_KLT_DUALMIC_EN_BIT);
	queue_work(led_wq,&audio_data.work);
	return;
}

void htcrhodium_microp_audio_codec_set(char on)
{
	printk(KERN_DEBUG "%s(%d)\n", __func__, on);
	if (on)
		audio_data.states |= RHOD_MICROP_KLT_CODEC_EN_BIT;
	else
		audio_data.states &= (~RHOD_MICROP_KLT_CODEC_EN_BIT);
	queue_work(led_wq,&audio_data.work);
	return;
...
static int htcrhodium_microp_audio_probe(struct platform_device *pdev)
{
	...
	mutex_unlock(&audio_data.lock);
	INIT_WORK(&audio_data.work, htcrhodium_microp_audio_leds_update);

	htcrhodium_microp_audio_codec_set(0);
	htcrhodium_microp_audio_dualmic_set(1);
	htcrhodium_microp_audio_dualmic_set(0);
	return 0;
}
...
int micropklt_dualmic_set(char on) {
	htcrhodium_microp_audio_dualmic_set(on);
	return 0;
}

int micropklt_codec_set(char on) {
	htcrhodium_microp_audio_codec_set(on);
	return 0;
}
