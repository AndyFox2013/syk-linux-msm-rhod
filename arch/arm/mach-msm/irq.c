...
static struct irq_data static_irq;
static struct irq_data * msm_to_irq_data(unsigned irq)
{
	static_irq.irq = irq;
	return &static_irq;
}

static void msm_irq_ack(struct irq_data *data)
{
...
static void msm_irq_mask(struct irq_data *data)
{
	unsigned irq = data->irq;
...
static void msm_irq_unmask(struct irq_data *data)
{
	unsigned irq = data->irq;
...
static int msm_irq_set_wake(struct irq_data *data, unsigned int on)
{
	unsigned irq = data->irq;
	unsigned index = __bank(irq);
	uint32_t mask = 1UL << (irq & 31);
...
static int msm_irq_set_type(struct irq_data *data, unsigned int flow_type)
{
	unsigned irq = data->irq;
...
void msm_irq_enter_sleep1(bool arm9_wake, int from_idle)
{
	if (!arm9_wake || !smsm_int_info)
		return;
	smsm_int_info->aArm_en_mask = msm_irq_smsm_wake_enable[!from_idle];
	smsm_int_info->aArm_interrupts_pending = 0;
}
...
int msm_irq_enter_sleep2(bool arm9_wake, int from_idle)
{
	...
	if (arm9_wake) {
		msm_irq_set_type(msm_to_irq_data(INT_A9_M2A_6), IRQF_TRIGGER_RISING);
		msm_irq_ack(msm_to_irq_data(INT_A9_M2A_6));
...
void msm_irq_exit_sleep1(void)
{
	...
	msm_irq_ack(msm_to_irq_data(INT_A9_M2A_6));
	msm_irq_ack(msm_to_irq_data(INT_PWB_I2C));
	for (i = 0; i < VIC_NUM_BANKS; i++) {
...
//		printk(KERN_ERR "msm_irq_exit_sleep <SM NO INT_INFO>\n");
	...
	if (msm_irq_debug_mask & IRQ_DEBUG_SLEEP) {
		printk(KERN_INFO "%s %x %x %x now", __func__,
			smsm_int_info->aArm_en_mask,
			smsm_int_info->aArm_interrupts_pending,
			smsm_int_info->aArm_wakeup_reason);
		print_vic_irq_stat();
	}
}
...
//		printk(KERN_ERR "msm_irq_exit_sleep <SM NO INT_INFO>\n");
	...
	pending = smsm_int_info->aArm_interrupts_pending;
...
void msm_fiq_enable(int irq)
{
	...
	irq_desc[irq].irq_data.chip->irq_unmask(msm_to_irq_data(irq));
...
void msm_fiq_disable(int irq)
{
	unsigned long flags;
	local_irq_save(flags);
	irq_desc[irq].irq_data.chip->irq_mask(msm_to_irq_data(irq));
	local_irq_restore(flags);
}
