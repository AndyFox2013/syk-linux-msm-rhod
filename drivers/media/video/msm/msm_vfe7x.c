...
#ifdef CONFIG_MSM_VFE_MSG_DECODE
#include "msm_vfe7x_dbg.h"
#endif

#include <asm/mach-types.h>

#define DEBUG_ADSP_COMMANDS
...
static uint8_t use_legend_lib = 1;      // Used to patch the command sent by the liboemcamera.so from Legend
static uint8_t vfe_is_running = 0;      // Used to block a command sent by the camera library from Legend
	...
	case CMD_STATS_DISABLE:{
			...
			if (vfecmd->queue == QDSP_CMDQUEUE) {
				...
				case VFE_START_CMD:
                    ...
                    vfe_is_running = 1;
                    printk("vfecmd->length = %d\n", vfecmd->length);
                    if ( vfecmd->length == 16 ) {
                        use_legend_lib = 1;
                        pr_info("Legend liboemcamera.so detected !\n");
                    } else {
                        use_legend_lib = 0;
                        pr_info("Sapphire/Dream liboemcamera.so detected !\n");
                    }
					break;
...
config_send:
    if ( machine_is_htcraphael() || machine_is_htcraphael_cdma() || machine_is_htcraphael_cdma500() ) {
       if ( (vfecmd->queue == 24) && ((*(uint32_t *) cmd_data) == 7) ) {
            CDBG("%s : Patching VFE_CMD_IP_FORMAT_CFG for raphael\n", __func__);
            ((uint32_t*) cmd_data)[1] = 0x03;
        }
    }

    if ( use_legend_lib ) {
        /* Patch for new liboemcamera.so */
        if ( (vfecmd->queue == QDSP_CMDQUEUE) && ((*(uint32_t *) cmd_data) == 13) ) {
#ifdef CONFIG_MSM_VFE_MSG_DECODE
            vfe_7x_decode_msg(vfecmd, cmd_data, 1);
#endif
            printk("%s : Patching VFE_CMD_ADP_FILTER_CFG for new lib -- dirty hack \n", __func__);
            ((uint32_t*) cmd_data)[1] = 0x0a03c065;
            ((uint32_t*) cmd_data)[2] = 0x00fcfcfc;
            ((uint32_t*) cmd_data)[3] = 0x00fc40fc;
            ((uint32_t*) cmd_data)[4] = 0x00fcfcfc;
            ((uint32_t*) cmd_data)[5] = 0x00070707;
            ((uint32_t*) cmd_data)[6] = 0x00070807;
            ((uint32_t*) cmd_data)[7] = 0x00070707;
        }

        if ( (vfecmd->queue == QDSP_CMDQUEUE) && ((*(uint32_t *) cmd_data) == 6) ) {
            printk("%s : Patching VFE_CMD_DEMOSAIC_CFG for new lib -- dirty hack \n", __func__);
            printk("lib sent 0x%08x\n", ((uint32_t*) cmd_data)[1]);
            ((uint32_t*) cmd_data)[1] = 0x00008102; 
            //vfecmd->length = 8;
        }

        /* This command makes vfe task crash when running (msg_id 3) */
        if ( (vfecmd->queue == QDSP_CMDQUEUE) && ((*(uint32_t *) cmd_data) == 6) ) {
            if (vfe_is_running) {
                printk("%s : Blocking command %d while vfe is running\n", __func__, *(uint32_t *) cmd_data);
                goto config_done;
            }
        }
        /* End of patch for new liboemcamera.so */
    }

	/* HTC: check cmd_data */
	if (cmd_data) {
#ifdef CONFIG_MSM_VFE_MSG_DECODE
        vfe_7x_decode_msg(vfecmd, cmd_data, 0);
# ifdef DEBUG_ADSP_COMMANDS
    	vfe_7x_display_command_id(vfecmd, cmd_data);
# else
