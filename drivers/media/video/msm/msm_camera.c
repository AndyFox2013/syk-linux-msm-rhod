		...
		struct msm_stats_event_ctrl *se)
{
	...
	if (!(sync->pp_mask & PP_PREV)) {
		pr_err("%s: diverting frame, not in PP_PREV!\n",
			__func__);
		return -EINVAL;
	}
		...
		struct msm_frame *pb)
{
	...
	if (pphy != 0) {
		CDBG("%s: rel: vaddr %lx, paddr %lx\n",
			__func__,
			(long unsigned int)pb->buffer, pphy);
		...
		gfp_t gfp)
{
    ...
    /* Print the errors data from VFE */
    if ( (qtype == MSM_CAM_Q_VFE_MSG) && 
            (vdata->evt_msg.msg_id == 13) ) {
        CDBG("%s : VFE_MSG_ERROR ! err_cond = 0x%08x, camif_sts = 0x%08x\n",
                __func__, ((int*)vdata->evt_msg.data)[0], ((int*)vdata->evt_msg.data)[1]);
    }
			...
			struct msm_sensor_ctrl *))
{
    ...
    /* Get camera node to register camera in the good order */
    struct msm_camera_sensor_info *sdata = dev->dev.platform_data;
    camera_node = sdata->sensor_node;
