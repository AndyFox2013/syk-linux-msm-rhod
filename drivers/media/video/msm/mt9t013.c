...
//HTC CSP start 20091208 gordon change for flicker
//	rc = mt9t013_i2c_write_w(mt9t013_client->addr,
...
static int32_t mt9t013_write_exp_gain(uint16_t gain, uint32_t line)
{
	...
	/*/HTC_CSP_START gordon 20091207 Add for Kevin add for solving AEC lock issue on Blackstone*/
 /*/        static uint16_t IT1;		//Kevin add for solving AEC lock issue 20080821*/
 /*/        static uint16_t gain3;		//Kevin add for solving AEC lock issue 20080821 //081117 Steven_Yeh changes gain2 to gain3 for HTC-TPE ISO re-static*/
        /*/HTC_CSP_END*/
...
/*/HTCCSP start*/
#if 0
 20091207 gordon add to 081120 Steven_Yeh Add for Kevin add for solving AEC lock issue on Blackstone
        rc = mt9t013_i2c_read_w(mt9t013_client->addr, REG_GLOBAL_GAIN, &gain3);
        if (rc < 0)
     		return rc;
        rc = mt9t013_i2c_read_w(mt9t013_client->addr, REG_COARSE_INT_TIME, &IT1);
        if (rc < 0)
     	 	return rc;
     
     
        if (IT1 != line || gain3 != gain ) //081117 Steven_Yeh changes gain2 to gain3 for HTC-TPE ISO re-static
        {
        rc = mt9t013_i2c_write_w(mt9t013_client->addr, REG_GROUPED_PARAMETER_HOLD, GROUPED_PARAMETER_HOLD);
        if (rc < 0)
		return rc;	
...
/*/HTCCSP END*/
	...
	rc = mt9t013_i2c_write_w(mt9t013_client->addr, REG_COARSE_INT_TIME, line);
	if (rc < 0)
		return rc;
#if 0
HTCCSP start
        rc = mt9t013_i2c_write_w(mt9t013_client->addr, REG_GROUPED_PARAMETER_HOLD, GROUPED_PARAMETER_UPDATE);
	if (rc < 0)
		return rc;
        }
        gain3= gain;
	...
	enum mt9t013_setting rt)
{
	...
	if (rt == RES_PREVIEW || rt == RES_CAPTURE) {
		...
		if (machine_is_sapphire() || machine_is_htcraphael() || 
            machine_is_htcraphael_cdma() || machine_is_htcraphael_cdma500() ) {
			if (rt == 0)
				rc = mt9t013_i2c_write_w(mt9t013_client->addr,
					REG_READ_MODE,
					0x046F);
	...
	if (rt == RES_PREVIEW || rt == RES_CAPTURE) {
		...
		rc =
			mt9t013_i2c_write_w(mt9t013_client->addr,
				0x3096, 0x4950); /*/ 20091203 gordon modify for errata 0x4949*/
		if (rc < 0)
			return rc;
...
/*CC091210~*/
	...
	{
		printk("%s: failed to get gpio\n", __func__);
		goto init_probe_done;
	}

	mdelay(20);

	printk("%s: sensor gpio reset\n", __func__);
...
static int32_t mt9t013_poweron_af(void)
{
	...
	if (1) { //(!rc) {
        if( !(mt9t013_ctrl->sensordata->vcm_reversed_polarity) ) {
	...
	const struct i2c_device_id *id)
{
	...
	if (machine_is_htcrhodium()) {
		if(get_machine_variant_type() < MACHINE_VARIANT_RHOD_2XX) {
			mt9t013_regs.reg_pat[RES_PREVIEW].pll_multiplier=60;
			mt9t013_regs.reg_pat[RES_CAPTURE].pll_multiplier=50;
		}
	}
		...
		struct msm_sensor_ctrl *s)
{
	...
	printk("%s: add: %x, client: %x\n", __func__, rc, (unsigned int)mt9t013_client);
...
#if 1
/*CC091210*/
	rc = gpio_request(info->sensor_reset, "mt9t013");
	if (!rc)   {
