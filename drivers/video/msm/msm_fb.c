...
static void power_on_panel(struct work_struct *work)
{
	...
	if (msmfb->sleeping == UPDATING) {
		wake_lock_timeout(&msmfb->idle_lock, HZ*5);
...
static void power_on_panel2(struct work_struct *work)
{
	int i;
	for(i=0; i < 10; i++)
		power_on_panel(work);
...
static int msmfb_ioctl(struct fb_info *p, unsigned int cmd, unsigned long arg)
{
	...
	case MSMFB_BLIT:
		...
		break;
	case MSMFB_UPDATE:
		if(copy_from_user(&rect,argp,sizeof(struct mdp_rect))==0)
			msmfb_pan_update(p, rect.x, rect.y, rect.w, rect.h,
				 0, 0);
		break;
	default:
...
static int msmfb_probe(struct platform_device *pdev)
{
	...
	//XXX: orig power_on_panel2, unclear why we need to do it 10x?
