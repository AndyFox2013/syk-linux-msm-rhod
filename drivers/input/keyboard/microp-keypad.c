...
extern int microp_ng_read_wrapper(char* name, uint8_t id, uint8_t *buf, int len);
...
{
	queue_work(led_wq, &microp_keypad.keypad_work);
	return IRQ_HANDLED;
}

static irqreturn_t microp_clamshell_interrupt(int irq, void *handle)
{
	queue_work(led_wq, &microp_keypad.clamshell_work);
	return IRQ_HANDLED;
}
...
static void microp_keypad_work(struct work_struct *work)
{
	...
	do {
		microp_ng_read_wrapper("ksc",MICROP_KSC_ID_SCANCODE, buffer, 2);

		key = buffer[0] & MICROP_KSC_SCANCODE_MASK;
		isdown = !(buffer[0] & MICROP_KSC_RELEASED_MASK);

		if (microp_keypad.pdata->read_modifiers) {
			microp_ng_read_wrapper("ksc",MICROP_KSC_ID_MODIFIER, buffer, 2);
